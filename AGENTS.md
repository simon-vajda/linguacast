...
- `apps/website` — the `hallspeak.app` Astro site. The homepage is a custom page (`src/pages/index.astro`, `src/layouts/HomeLayout.astro`, `src/components/home`) styled only from the tokens in `src/styles/home.css`: every homepage length comes from a token there (spacing on a 4px step plus one 2px half-step; fluid values are `clamp()`s between tokens). Literal lengths are limited to 1px/2px borders, media-query breakpoints and image `sizes` hints, which cannot read custom properties. The operator guide and privacy policy are Starlight pages themed by `src/styles/theme.css`. Both share Starlight's `starlight-theme` storage key, so a theme choice carries across. No `typecheck` script (`@astrojs/check` would make TypeScript 5 reachable); Astro transpiles its one `.ts` file, `content.config.ts`, without type-checking it. `astro build`, with its link validator, is its check. Deployed to GitHub Pages by `.github/workflows/website.yml`.
...
- **Server logging is levelled, and the default level is a budget.** A record sits at `info` or above only if it answers a check in `docs/solutions/operations/diagnosing-live-audio-from-a-user-report.md` or places a failure on its capacity ladder. Connection and transport narration is `debug`; anything naming a listener — a forwarded address, a remote candidate address — is `trace`, which is also written to disk for the retention window. `LOG_LEVEL` defaults to `info`. Cost at the default is flat per event: no `info` record per listener, so listener volume appears only as the count at each on-air and off-air boundary and the peak between them. Short transport identifiers are `debug` or lower. No record at any level, server or client, carries a PIN, speaker code, session identifier or password — passed as fields, those are redacted structurally; interpolated into a message they are not, which is why values are fields and messages are short static strings. The runbook and this rule are written against each other; neither drifts alone.
